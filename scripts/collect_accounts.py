...
Collects TikTok videos from official party accounts via the Research API.
Also fetches user stats (follower_count etc.) for each account at scrape time.
...
Populate data/accounts.csv with columns: username, party

Usage
-----
    python collect_accounts.py              # collect all accounts
    python collect_accounts.py --status     # show progress without collecting
    python collect_accounts.py --merge      # merge per-account CSVs into combined.csv

Output
------
    data/collected/<username>.csv   — raw videos per account
    data/collected/progress.json    — tracks completed accounts
    data/combined.csv               — merged deduplicated file (after --merge)
"""
...
END_DATE   = "2024-07-18"
CHUNK_DAYS = 28   # API enforces ≤30-day windows per request

MAX_PER_REQUEST    = 100
REQUESTS_PER_DAY   = 1000
SLEEP_BETWEEN      = 2.0

OUTPUT_DIR    = os.path.join(ROOT_DIR, "data", "raw", "accounts")
PROGRESS_FILE = os.path.join(OUTPUT_DIR, "progress.json")
ACCOUNTS_FILE = os.path.join(ROOT_DIR, "data", "supplementary", "accounts.csv")
...
def fetch_videos_chunk(token, username, start, end, progress):
    """Fetch all videos for an account within a single ≤30-day window.

    Increments progress["request_count_today"] on every HTTP call (including
    retries) and saves progress after each call, so a mid-account crash never
    loses the quota counter. Wraps requests.post in try/except so a transient
    network error retries instead of aborting the whole account.
    """
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    all_videos = []
    cursor     = 0
...
def fetch_videos(token, username, progress):
    """Fetch all videos for an account across the full date range, chunked into ≤30-day windows."""
...
def save_account(videos, username, party, user_info, scrape_ts):
    if not videos:
        return 0
    df = pd.DataFrame(videos)
    # Stringify music_id from the original Python dicts (exact ints from JSON) before
    # pandas can float64-coerce large IDs when any row has a null music_id.
...
def merge_all():
    ...
    out_path = os.path.join(ROOT_DIR, "data", "combined", "accounts_combined.csv")
...
def main():
    ...
    if accounts.empty:
        print("No accounts in data/accounts.csv — add rows with username,party and retry.")
        return
    ...
    if args.status:
        ...
        print(f"Progress: {done}/{total} accounts collected")
        print(f"Requests today: {progress['request_count_today']}/{REQUESTS_PER_DAY}\n")
        for _, row in accounts.iterrows():
            status = "✓" if row["username"] in progress["completed"] else "○"
            print(f"  {status} {row['username']} ({row['party']})")
        return
    ...
    print(f"Collecting {len(accounts)} accounts")
    ...
    for _, row in accounts.iterrows():
        ...
        print(f"  Fetching @{username} ({party})...", end=" ", flush=True)
        ...
        try:
            ...
            n_saved = save_account(videos, username, party, user_info, scrape_ts)
            progress["completed"].append(username)
            save_progress(progress)
            n_requests = progress["request_count_today"] - requests_before
            print(f"{n_saved} videos ({n_requests} requests, {progress['request_count_today']} today)")
